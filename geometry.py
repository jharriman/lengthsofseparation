...
def createConnections(sequence, maxLength):
    ...
    for s in sequence:
        ...
        if (not reverse and (currentPosition > maxLength)) or (reverse and (currentPosition < maxLength)):
            ...
            reverse = not reverse
        if not reverse:
            output += "translate([%f, %f, 0]) cube([%f, %f, %f]);" % (yAdjustment, currentPosition, cubeSize, s * 100, cubeSize)
            output += "translate([%f, %f, 0]) cube([%f, %f, %f]);" % (yAdjustment, currentPosition + (s * 100), interludeSize, separation, interludeSize)
            currentPosition += (s * 100) + separation
        else:
            output += "translate([%f, %f, %f]) rotate([180, 0 ,0]) cube([%f, %f, %f]);" % (yAdjustment, currentPosition, cubeSize, cubeSize, s * 100, cubeSize)
            output += "translate([%f, %f, %f]) rotate([180, 0, 0]) cube([%f, %f, %f]);" % (yAdjustment, currentPosition - (s * 100), interludeSize, interludeSize, separation, interludeSize)
            currentPosition -= (s * 100) + separation
